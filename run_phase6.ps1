...
$Configs = @("burst_001", "periodic_001", "mixed_001")
$Policies = @("reactive", "fixed", "threshold", "predictive_ema", "predictive_hybrid")

New-Item -ItemType Directory -Force -Path ".\experiments\results\phase6_live" | Out-Null
...
foreach ($policy in $Policies) {
    foreach ($config in $Configs) {
        ...
        $check_file = ".\experiments\results\phase6_live\${policy}_${config}_health.txt"
        .\venv\Scripts\python.exe .\tests\check_leaks.py > $check_file
        
        # Generate Report
        $report_file = ".\experiments\results\phase6_live\${policy}_${config}_report.txt"
...
Write-Host "Phase 6 Live Predictive Benchmarks Completed!"
