...
$Policies = @("reactive", "fixed", "threshold")
...
foreach ($policy in $Policies) {
    foreach ($config in $Configs) {
        ...
        # Register Compute (Only used in mixed, but safe to register always)
        $payload_compute = @{
            name = "compute"
            ...
            idle_timeout_seconds = 300
            scheduling_policy = $policy
            min_containers = if ($policy -eq "fixed") { 2 } else { 0 } # Different limit for compute
            queue_threshold = $queue_threshold
        }
        ...
        # For fixed, give it a second to pre-warm
        if ($policy -eq "fixed") {
        ...
        # Generate Report
        $report_file = ".\experiments\results\phase4_${policy}_${config}.txt"
...
Write-Host "Phase 4 Policy Experiments Completed!"
