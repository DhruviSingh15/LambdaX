...
$raw_dir = "$out_dir\raw"
$csv_path = "$out_dir\phase8_raw_results.csv"
    ...
    foreach ($wl in $workloads) {
        foreach ($policy in $policies) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                # Export raw to run_dir
                # Generate stats and append to CSV
                # For simplicity, using reporter.py to parse and append CSV format
...
Write-Host "Phase 8 Matrix Completed!"
